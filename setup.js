...
function buildSecretHighlightSvg(word){
  ...
  // Rotated 90°, a glyph's horizontal extent equals its upright vertical
  // extent — roughly fontSize × 1.2 including descenders. Cap fontSize
  // well below tileW so rotated letters sit centered with breathing room
  // instead of being cropped against the tile edges.
...
function setupSecretHighlight(){
  ...
  function render(){
    ...
    var lcHeight = lcRect.height;
    if(lcWidth <= 0) return;
    var vh = lcWidth * tileAspect;
    // Only allow whole repetitions of the watermark — never a partial
    // tail at the bottom. maxY is the inclusive upper bound of the Y
    // region where watermark may be painted (relative to letter-content).
    // Math.max(1, …) keeps the first rep alive on viewports shorter than
    // one rep (desktop) where Math.floor would otherwise give 0.
    var reps = Math.max(1, Math.floor(lcHeight / vh));
    var maxY = reps * vh;
    for(var i = 0; i < rects.length; i++){
      var r = rects[i];
      if(r.width <= 0 || r.height <= 0) continue;
      // Skip rects that don't overlap the letter at all — Ctrl+A on the
      // page selects outside elements too; we want watermark ONLY where
      // the selection intersects #letter-content.
      if(r.right <= lcRect.left || r.left >= lcRect.right) continue;
      if(r.bottom <= lcRect.top || r.top >= lcRect.bottom) continue;
      // Intersect with letter-content so rects that extend outside it
      // (e.g. selection wrapping past the right margin) get clipped.
      var clippedLeft = Math.max(r.left, lcRect.left);
      var clippedTop = Math.max(r.top, lcRect.top);
      var clippedRight = Math.min(r.right, lcRect.right);
      var clippedBottom = Math.min(r.bottom, lcRect.bottom);
      var localLeft = clippedLeft - lcRect.left;
      var localTop = clippedTop - lcRect.top;
      var localRight = clippedRight - lcRect.left;
      var localBottom = clippedBottom - lcRect.top;
      // Respect the "no partial rep at bottom" rule — clip against maxY.
      if(localTop >= maxY) continue;
      if(localBottom > maxY) localBottom = maxY;
      var widthPx = localRight - localLeft;
      var heightPx = localBottom - localTop;
...
// ── SCROLL-EXTEND-SELECTION ─────────────────────────────────────────
// Native quirk on desktop: while the mouse button is held down to
// extend a selection, scrolling with the wheel/trackpad does NOT
// extend the selection — the browser only re-hit-tests on actual
// mouse movement. The user has to wiggle the mouse a pixel for the
// selection to grow. To fix that we track the last pointer position
// and, on scroll while a mouse button is held, dispatch a synthetic
// mousemove at that same screen position. The browser re-hit-tests
// against the now-scrolled document and extends the selection.
// Touch pointers are ignored (mobile selection uses different gesture
// mechanics). Scope: letter view only. Revert: delete this block, the
// openLetter call site, and the logout cleanup call.
var scrollExtendSelectionCleanup = null;
function setupScrollExtendSelection(){
  if(scrollExtendSelectionCleanup){ scrollExtendSelectionCleanup(); }
  if(isAdmin) return;
  var mouseDown = false;
  var lastX = 0, lastY = 0;
  ...
  function onScroll(){
    ...
    try {
      var ev = new MouseEvent("mousemove", {
        ...
        clientX: lastX, clientY: lastY, button: 0, buttons: 1
      });
  ...
  scrollExtendSelectionCleanup = function(){
    document.removeEventListener("pointerdown", onPointerDown, true);
    document.removeEventListener("pointermove", onPointerMove, true);
    document.removeEventListener("pointerup", onPointerUp, true);
    document.removeEventListener("pointercancel", onPointerUp, true);
    window.removeEventListener("scroll", onScroll, true);
    scrollExtendSelectionCleanup = null;
  };
}
// ── /SCROLL-EXTEND-SELECTION ────────────────────────────────────────
...
function openLetter(){
  ...
  // SCROLL-EXTEND-SELECTION (delete next line to revert)
  setupScrollExtendSelection();
...
function logout(){
  ...
  // SCROLL-EXTEND-SELECTION (delete next line to revert)
  if(typeof scrollExtendSelectionCleanup === "function") scrollExtendSelectionCleanup();
