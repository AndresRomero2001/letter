...
.papyrus{
  position:relative;width:100%;padding:32px 0;
  filter:drop-shadow(0 22px 50px rgba(10,30,70,.45));
}
/* Horizontal cylinder viewed from the side — paper rolled around a rod */
.papyrus-roll{
  position:absolute;left:-14px;right:-14px;height:34px;z-index:3;
  pointer-events:none;
  /* Stepped bands simulate pixelated side-profile curvature of rolled paper */
  background:
    linear-gradient(180deg,
      ...
      #4b8dba 76%,  #4b8dba 86%,
      #2c5b80 86%,  #2c5b80 94%,
      #153a5c 94%,  #153a5c 100%);
  border-radius:3px;
  box-shadow:
    0 6px 14px rgba(10,30,70,.45),
    inset 0 1px 0 rgba(255,255,255,.45),
    inset 0 -1px 0 rgba(8,22,45,.6);
  image-rendering:pixelated;
}
/* Thin horizontal lines = edges where paper wraps the cylinder */
.papyrus-roll::after{
  content:"";position:absolute;top:0;bottom:0;left:18px;right:18px;pointer-events:none;
  background:
    linear-gradient(180deg,
      transparent 24%,
      rgba(255,255,255,.22) 25%, rgba(255,255,255,.22) 27%,
      transparent 28%,
      transparent 62%,
      rgba(10,25,50,.4) 63%, rgba(10,25,50,.4) 65%,
      transparent 66%);
}
/* LEFT end-cap: small vertical ellipse showing the spiral of rolled paper from the side */
.papyrus-roll::before{
  content:"";position:absolute;top:-5px;bottom:-5px;left:-13px;width:24px;
  pointer-events:none;border-radius:50%;
  background:
    radial-gradient(ellipse 60% 55% at 60% 50%,
      #0e2642 0 10%,
      #1c4165 10% 20%,
      #3b6d96 20% 30%,
      #1c4165 30% 36%,
      ...
      #0e2642 92% 100%);
  box-shadow:
    0 4px 10px rgba(10,30,70,.55),
    inset 1px 0 2px rgba(255,255,255,.35),
    inset -1px 0 2px rgba(8,22,45,.5);
}
/* RIGHT end-cap (mirror of left — uses element because ::before/::after taken) */
.papyrus-roll-right{
  position:absolute;top:-5px;bottom:-5px;right:-13px;width:24px;
  pointer-events:none;border-radius:50%;z-index:4;
  background:
    radial-gradient(ellipse 60% 55% at 40% 50%,
      #0e2642 0 10%,
      #1c4165 10% 20%,
      #3b6d96 20% 30%,
      #1c4165 30% 36%,
...
.papyrus-body{
  ...
  background:
    linear-gradient(180deg,
      rgba(200,230,248,0.14) 0%,
      rgba(168,213,237,0.20) 40%,
      rgba(200,230,248,0.14) 100%);
  backdrop-filter:blur(14px) saturate(150%);
  -webkit-backdrop-filter:blur(14px) saturate(150%);
  border-left:1px solid rgba(200,230,248,.35);
  border-right:1px solid rgba(200,230,248,.35);
  box-shadow:
    inset 0 0 100px rgba(120,180,230,.14),
    inset 50px 0 80px -55px rgba(200,230,248,.18),
    inset -50px 0 80px -55px rgba(200,230,248,.18),
    inset 0 24px 40px -25px rgba(230,245,255,.18),
    0 2px 0 0 rgba(40,80,130,.3);
  padding:68px 56px 78px;
  min-height:62vh;
  overflow:hidden;
}
/* Minecraft stained-glass: scattered pixel squares on a 16x16 grid */
.papyrus-body::before{
  content:"";position:absolute;inset:0;pointer-events:none;opacity:.85;
  background-image:url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' shape-rendering='crispEdges'><rect width='16' height='16' fill='rgba(255,255,255,0)'/><rect x='0' y='0' width='1' height='1' fill='rgba(255,255,255,0.32)'/><rect x='5' y='1' width='1' height='1' fill='rgba(168,213,237,0.45)'/><rect x='11' y='0' width='1' height='1' fill='rgba(255,255,255,0.22)'/><rect x='3' y='3' width='1' height='1' fill='rgba(122,184,220,0.4)'/><rect x='8' y='4' width='1' height='1' fill='rgba(255,255,255,0.38)'/><rect x='14' y='3' width='1' height='1' fill='rgba(200,230,248,0.36)'/><rect x='1' y='6' width='1' height='1' fill='rgba(168,213,237,0.3)'/><rect x='6' y='7' width='1' height='1' fill='rgba(255,255,255,0.3)'/><rect x='12' y='6' width='1' height='1' fill='rgba(122,184,220,0.42)'/><rect x='4' y='9' width='1' height='1' fill='rgba(255,255,255,0.28)'/><rect x='9' y='10' width='1' height='1' fill='rgba(168,213,237,0.4)'/><rect x='15' y='9' width='1' height='1' fill='rgba(255,255,255,0.35)'/><rect x='2' y='12' width='1' height='1' fill='rgba(200,230,248,0.4)'/><rect x='7' y='13' width='1' height='1' fill='rgba(122,184,220,0.34)'/><rect x='13' y='12' width='1' height='1' fill='rgba(255,255,255,0.3)'/><rect x='5' y='15' width='1' height='1' fill='rgba(168,213,237,0.3)'/><rect x='10' y='14' width='1' height='1' fill='rgba(255,255,255,0.26)'/></svg>");
  background-size:48px 48px;
  image-rendering:pixelated;
  image-rendering:crisp-edges;
  mix-blend-mode:screen;
}
/* Pixelated grid overlay = faint minecraft block outlines */
.papyrus-body::after{
  ...
  background:
    ...
    radial-gradient(ellipse at 28% 18%, rgba(235,246,253,.18), transparent 45%),
    radial-gradient(ellipse at 75% 85%, rgba(180,220,245,.12), transparent 50%);
}
.letter-content{
...
@media(max-width:520px){
  ...
  h1{font-size:1.2rem}
  .papyrus-body{padding:54px 24px 62px}
  .letter-content{font-size:1.05rem;line-height:1.85;text-align:left}
  .letter-content p{text-indent:1.2em}
  .papyrus-roll{height:28px;left:-10px;right:-10px}
  .papyrus-roll::before,.papyrus-roll-right{width:20px;top:-4px;bottom:-4px}
  .papyrus-roll::before{left:-11px}
  .papyrus-roll-right{right:-11px}
  .papyrus{padding:26px 0}
}
</style>
</head>
<body>
